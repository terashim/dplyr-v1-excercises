...
`gropu_by()` によってデータ本体は変わらず、グループ化されたことを表す情報表示 `# Groups: carrier [16]`
が出る。
...
`summarise()` の後にグループ化の状態を残したくないとき、オプションで `.groups = "drop"`
...
`.groups = "drop"` の他にも, `.groups = "keep"`（グループ化の状態を変えない）, `.groups =
"rowwise"`（すべての行を個別にグループ化）の選択肢がある。

バージョン 1.0.0 では、 **`.groups` はまだ実験的
([experimental](https://www.tidyverse.org/lifecycle/#experimental)) な
API という位置づけになっている** ので、今後のバージョンで変更される可能性がある。
